...
// Components
import Toast from '../components/ui/Toast';
import AttendanceConfirmationToast from '../components/attendance/AttendanceConfirmationToast';
...
function Attendance() {
  ...
  // Success / Error toasts — unchanged, still powered by src/components/ui/Toast.jsx
  const [toast, setToast] = useState({
    show: false,
    message: '',
    type: 'success',
  ...
  // Confirmation toast — separate state, powered by the new local
  // AttendanceConfirmationToast component (pure React, no Bootstrap Toast JS).
  ...
  // ===============================
  // Toast Helpers (success / error only)
  // ===============================
  const showToast = (message, type = 'success') => {
    setToast({
      show: true,
  ...
  const showConfirmationToast = (err, requestData, apiFunction, action) => {
    setConfirmation({
      show: true,
      message: `You are currently checked in at "${err.response.data.previousBranch.name}". Continuing will invalidate that attendance and create a new check-in here.`,
      confirmText: 'Continue',
      cancelText: 'Cancel',
      onConfirm: () => confirmForcedAction(requestData, apiFunction, action),
    });
...
{deviceInfo && (
  ...
  <div className="attendance-device-badge">
    ...
    <div className="attendance-device-text">
      ...
      {/* Success / Error toast — unchanged, still Bootstrap-based Toast.jsx */}
      <Toast
        show={toast.show}
        message={toast.message}
        type={toast.type}
      ...
      {/* Confirmation toast — new, pure-React, local to Attendance */}
      {confirmation.show && (
        <AttendanceConfirmationToast
