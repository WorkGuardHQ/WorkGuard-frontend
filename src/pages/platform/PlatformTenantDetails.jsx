...
import { useParams, useNavigate }           from 'react-router-dom';
import { getTenant, updateTenant, updateTenantStatus } from '../../services/platform/platformTenants.service';
import { getRenewalHistory } from '../../services/platform/platformSubscriptionRenewal.service';
import PlatformLayout from '../../components/platform/PlatformLayout';
import RenewalModal   from './RenewalModal';
import TenantBillingCard
from '../../components/platform/TenantBillingCard';
import Toast          from '../../components/ui/Toast';
...
function RenewalHistorySection({ tenantId }) {
  const [rows, setRows]       = useState([]);
  const [loading, setLoading] = useState(true);
  const [loaded, setLoaded]   = useState(false);

  // ✅ Lazy load — نجيبها مرة واحدة بس أول ما القسم يتعرض فعليًا
  useEffect(() => {
    if (loaded) return;
    getRenewalHistory(tenantId, { limit: 10 })
  ...
  return (
    <Section title="Renewal History" icon="fa-file-invoice-dollar">
      {loading ? (
        ...
        <div className="table-responsive rounded-3">
          <table className="table table-sm mb-0" style={{ color: '#e2e8f0' }}>
            <thead>
              <tr style={{ borderColor: '#334155' }}>
                {['Plan', 'Type', 'Extras', 'Total', 'Payment', 'By', 'Date'].map(h => (
                  <th key={h} style={{ color: '#64748b', borderColor: '#334155', fontWeight: 500, fontSize: 12 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r._id} style={{ borderColor: '#334155' }}>
                  <td style={{ borderColor: '#334155', fontSize: 12 }}>
                    <span className={`badge bg-${PLAN_COLORS[r.planSlug] || 'secondary'}`}>{r.planName}</span>
                  </td>
                  <td style={{ borderColor: '#334155', fontSize: 12, textTransform: 'capitalize' }}>{r.renewalType}</td>
                  <td style={{ borderColor: '#334155', fontSize: 12, color: '#94a3b8' }}>
                    {(r.extras?.employees || r.extras?.branches || r.extras?.admins)
                      ? `+${r.extras.employees || 0} emp / +${r.extras.branches || 0} br / +${r.extras.admins || 0} adm`
                      : '—'}
                  </td>
                  <td style={{ borderColor: '#334155', fontSize: 12, fontWeight: 700 }}>{r.totalAmount} {r.currency}</td>
                  <td style={{ borderColor: '#334155' }}>
                    <span className={`badge ${r.paymentStatus === 'paid' ? 'bg-success' : 'bg-warning text-dark'}`} style={{ fontSize: 10 }}>
                      {r.paymentStatus}
                    </span>
                    <span className="badge bg-secondary ms-1" style={{ fontSize: 10 }}>{r.paymentMethod}</span>
                  </td>
                  <td style={{ borderColor: '#334155', fontSize: 12, color: '#94a3b8' }}>{r.renewedBy?.name || '—'}</td>
                  <td style={{ borderColor: '#334155', fontSize: 12, color: '#64748b' }}>{fmtDate(r.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <PlatformLayout>
      <div style={{ maxWidth: 900 }}>
        ...
        <div className="d-flex align-items-center gap-3 mb-4">
          ...
          {/* Actions */}
          <div className="d-flex gap-2">
            {/* ✅ جديد — Renew / Change Plan */}
            <button className="btn btn-sm btn-outline-info" onClick={() => setShowRenewal(true)}>
              <i className="fas fa-sync-alt me-1" />Renew / Change Plan
            </button>
            <button className="btn btn-sm btn-outline-primary" onClick={() => setShowEdit(true)}>
              <i className="fas fa-edit me-1" />Edit
            </button>
        ...
        <div className="row g-3 mb-4">
          {[
            { label: 'Active Employees', value: tenant.employeeCount, icon: 'fa-users',       color: 'info'    },
            { label: 'Active Admins',    value: tenant.adminCount,    icon: 'fa-user-shield', color: 'warning' },
            // ✅ Bug fix: كان sub?.plan (undefined دايمًا بعد rename الحقل لـ planSlug)
            { label: 'Plan',             value: sub?.planSlug || '—', icon: 'fa-box',         color: 'primary' },
            { label: 'Days Left',        value: expired ? 'Expired' : (days != null ? `${days}d` : '—'),
                                                                       icon: 'fa-calendar',    color: expired ? 'danger' : 'success' },
          ].map(({ label, value, icon, color }) => (
            <div className="col-6 col-md-3" key={label}>
        ...
        <Section title="Company Information" icon="fa-building"
          ...
          <InfoRow label="Created">{fmtDTime(tenant.createdAt)}</InfoRow>
          {/* ✅ عرض إضافي — إجمالي كل من مر على الشركة (وليس النشط فقط) */}
          {(tenant.totalEmployees != null) && (
            <InfoRow label="Total Ever (All-time)">
              <span style={{ fontSize: 12, color: '#94a3b8' }}>
                {tenant.totalEmployees} employees ever · {tenant.totalAdmins} admins ever
              </span>
            </InfoRow>
          )}
          {tenant.internalNotes && (
        ...
        <Section title="Current Subscription" icon="fa-credit-card"
          action={
            <button className="btn btn-sm btn-outline-info py-0 px-2" onClick={() => setShowRenewal(true)}>
              <i className="fas fa-sync-alt me-1" />Renew
            </button>
          }>
          {sub ? (
            <>
              <InfoRow label="Plan">
                {/* ✅ Bug fix: sub.planSlug بدل sub.plan */}
                <span className={`badge bg-${PLAN_COLORS[sub.planSlug] || 'secondary'}`}>
                  {sub.planSlug}
                </span>
                {sub.isTrial && <span className="badge bg-info bg-opacity-25 text-info ms-1">Trial</span>}
              </InfoRow>
              ...
              {sub.limits && (
                <InfoRow label="Limits">
                  <span style={{ fontSize: 12 }}>
                    ...
                    <i className="fas fa-code-branch me-1 text-info" />{sub.limits.maxBranches ?? '∞'} branches
                    <span className="mx-2">·</span>
                    <i className="fas fa-user-shield me-1 text-info" />{sub.limits.maxAdmins ?? '∞'} admins
                  </span>
                </InfoRow>
              )}
            </>
...
<TenantBillingCard
 ...
 />
              ...
              <table className="table table-sm mb-0" style={{ color: '#e2e8f0' }}>
                ...
                <tbody>
                  {tenant.subHistory.map((s, i) => (
                    <tr key={i} style={{ borderColor: '#334155' }}>
                      <td style={{ borderColor: '#334155' }}>
                        {/* ✅ Bug fix: s.planSlug بدل s.plan */}
                        <span className={`badge bg-${PLAN_COLORS[s.planSlug] || 'secondary'}`}>{s.planSlug}</span>
                        {s.isTrial && <span className="badge bg-info bg-opacity-25 text-info ms-1" style={{ fontSize: 10 }}>Trial</span>}
                      </td>
                      <td style={{ borderColor: '#334155' }}>
                        <span className={`badge bg-${s.status === 'active' ? 'success' : s.status === 'expired' ? 'danger' : 'secondary'}`}>
        ...
        {/* ── Renewal History (السجل المالي — Lazy loaded) ── */}
        <RenewalHistorySection tenantId={tenant._id} />
